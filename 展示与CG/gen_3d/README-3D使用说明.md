...
十二个角色的 3D 模型。前四个（zw/wy/xz/xm）由 `展示与CG/精英展示-04-圣火单卡旋转-GLM5.3.html` 里的立绘做图生 3D；其余八个（谢逊/周芷若·露肩/周芷若·黑化/宋青书/宋远桥/灭绝师太/张三丰/周芷若·湿身纱）项目内**无源图**，由 ImageGen 生成国风写实立绘（`立绘/`）后再图生 3D。
...
| 周芷若 · 湿身纱 (zzrws) | `zzrws/zzrws.glb`（47MB） | `zzrws/zzrws_web.glb`（1.5MB） | `zzrws/zzrws_viewer.html` |
...
- `3d-gallery.html`：十二合一画廊（多文件版），用压缩版，秒开。
...
### 方式 A：双击单文件版（最省事，推荐）
`gen_3d/gallery-standalone.html` 是**专为双击设计的单文件版**：查看器库和 12 个模型全部内联进这一个 HTML，
直接双击就能在浏览器里看，**不需要服务器**。库已内联所以不依赖 CDN；首次加载 Draco 解码器需联网（之后浏览器缓存，再开就全自动离线）。
文件较大（约 24.8MB），好处是拷到任何地方、双击即开。
...
- 2026-09-26 北京 17:12（UTC 09-26 日额度）：张三丰 3D 提交成功并生成完毕（3 分 03 秒，30 积分），高模 `zf/zf.glb`（47MB）+ 压缩版 `zf/zf_web.glb`（1.6MB）+ 预览 + 查看页 + OBJ 包；JobId `1495351759874293760`。
- 2026-09-26 北京 18:34（UTC 09-26 日额度）：周芷若·水雾湿身纱（最大尺度版）3D 提交成功并生成完毕（216 秒），高模 `zzrws/zzrws.glb`（47MB）+ 压缩版 `zzrws/zzrws_web.glb`（1.5MB）+ 预览 + 查看页 + OBJ 包；JobId `1495372146351751168`。原「汉服 cosplay」方案（`zzrhf`）因服务端超时失败（扣费 0），按用户要求改用最大尺度版。
  **当前额度**：UTC 09-26 日已用 3 次（灭绝、张三丰、周芷若·湿身纱），**还剩 2 次**（下次刷新：北京 09-27 08:00）。
  **待办**：宋青书 3D 需按新「武当苦练玉面版」立绘重做（现 `sqs.glb` 仍是旧黑化版）；玄冥二老（鹿杖客/鹤笔翁，占 2 次）、成昆的 3D 待用户从 2D 中挑选后提交。
- `gallery-standalone.html` 已重建：内联 12 个模型（含周芷若·湿身纱），双击即开（约 24.8MB）；`3d-gallery.html` 为多文件版，需本地 HTTP 服务。
- 2026-09-26 起：`gen_3d/` 的**立绘 PNG + 网页版 `_web.glb` + 预览 + viewer** 已入库 GitHub（`origin`，`main`），手机端可直接查看；原始大模型（47MB×N）、OBJ 包、`gallery-standalone.html`（24MB）、`_jobs/`（含带签名的下载链接，会触发 GitHub secret scanning）均排除。
