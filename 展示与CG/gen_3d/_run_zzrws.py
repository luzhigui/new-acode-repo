# -*- coding: utf-8 -*-
"""周芷若·水雾湿身纱(大尺度) → 图生3D（提交+轮询+下载 一条龙）"""
...
IMG = os.path.join(HERE, "立绘", "周芷若-水雾湿身纱-大尺度性感-v1.png")
NAME = "zzrws"                      # 周芷若·水雾湿身纱
...
T = ('<!DOCTYPE html><html lang="zh"><head><meta charset="utf-8">'
     ...
     '<title>周芷若·水雾湿身纱 3D</title><style>html,body{margin:0;height:100%;background:#12141a;}'
     'model-viewer{width:100%;height:100%;}</style></head><body>'
     '<script type="module" src="../model-viewer.min.js"></script>'
     '<model-viewer src="zzrws.glb" camera-controls auto-rotate shadow-intensity="1" '
     'exposure="1" interaction-prompt="none" alt="周芷若·水雾湿身纱"></model-viewer></body></html>')
with io.open(os.path.join(OUT, NAME + "_viewer.html"), "w", encoding="utf-8") as f:
    f.write(T)
saved.append(os.path.join(OUT, NAME + "_viewer.html"))

print("DONE", JOB, json.dumps([os.path.relpath(p, HERE) for p in saved], ensure_ascii=False))
