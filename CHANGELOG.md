...
### 工程维护
- **工具路径同步更新**：文件复制器 `tools/32-toolkit.js` 的 `ALL_PROJECT_FILES` 与函数提取器 `tools/33-toolkit-more.js` 的 `TARGET_FILES` 同步调整精英角色模块路径，移除旧的 `modules/91elite-xiaozhao-brother.js` ~ `modules/99elite-zhangwuji.js`，新增注册 `modules/97elite-imperial.js`、`modules/98elite-sixsects.js`、`modules/99elite-mingjiao.js`，并新增注册 `core/00-event-bus.js`
