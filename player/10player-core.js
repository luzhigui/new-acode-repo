...
// V4.0.1 | 2026-06-29 拆分playLogEntries | ~550 lines | ~15000 字符
export const VER = 'player/10player-core.js V4.0.1';
...
async function handleAttackGroup(c, entry, roundResult, abortSig, isFirstAttackInRoundRef) {
    ...
    if (unitA) {
        ...
        if (uiUnitA) {
            ...
            uiUnitA.buffDefBonus = unitA.buffDefBonus || 0;
            uiUnitA.buffHpBonus = unitA.buffHpBonus || 0;
        }
    }
    if (unitD) {
        ...
        if (uiUnitD) {
            ...
            uiUnitD.buffDefBonus = unitD.buffDefBonus || 0;
            uiUnitD.buffHpBonus = unitD.buffHpBonus || 0;
        }
    }
        ...
        for (let i = 0; i < log.length; i++) {
            ...
            switch (entry.type) {
                ...
                case 'buff-splash':
                    ...
                    break;
                case 'buff-bonus':
                    await handleBuffBonus(c, entry);
                    break;
                case 'buff-swap':
                    await handleBuffSwap(c, entry);
                    break;
                case 'buff-push':
                ...
                case 'buff-rebound-fortify':
                    ...
                    break;
                case 'round-start':
                    await handleRoundStart(c, entry, isFirstAttackInRoundRef);
                    break;
                    break;
                case 'attack-group': {
                ...
                case 'info':
                    ...
                    break;
                case 'round-end':
                    await handleRoundEnd(c, entry, log, i);
                    break;
            }
            if (abortSig && abortSig.aborted) return { isBattleOver: false };
        }
