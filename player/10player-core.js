...
export async function playBattle() {
    ...
    if (winner === '明教' || winner === '六大派') { c.battleResultForInfo = { winner, ally: c.UI.allyTeam, enemy: c.UI.enemyTeam }; let units = winner === '明教' ? c.UI.allyTeam : c.UI.enemyTeam, alive = units.filter(u => u.alive); if (alive.length > 0) { alive.forEach(u => { u._flash = 'cheer'; }); c.updateUI(c.UI); let victoryLines = ['我们赢了！','好！','胜了！','痛快！','拿下！','赢了！']; alive.forEach((u,i) => { setTimeout(() => { let line = victoryLines[i % victoryLines.length]; let div = document.createElement('div'); div.innerHTML = '<span class="gold">🗣️ ' + u.name + '：' + line + '</span><br>'; logDiv.appendChild(div); c.autoScrollLog(); }, i * 400); }); await new Promise(r => setTimeout(r, 800 + alive.length * 400)); if (c.spawnVictoryEffects) c.spawnVictoryEffects(winner); await new Promise(r => setTimeout(r, 3000)); } } else { logDiv.innerHTML+='<span class="gray">🤝 平局！积分不变</span><br>'; logDiv.scrollTop = logDiv.scrollHeight; }
