// tools/71-ai-pack-config.js - 光明顶5v5 AI 复制包配置（清单/分组/提示词/精简踢除）
// V5.4.1 | ~10500 bytes| 2026-08-15 从 49-toolkit.js 拆出静态数据，新增 AI_EXCLUDE
export const VER = 'tools/71-ai-pack-config.js V5.4.1';

// ==================== AI 精简模式踢除清单（第一批） ====================
// 这些文件不参与战斗逻辑/状态同步/数值结算，默认不随包发送。
// 改动画实现/音效/错误面板/入口页时，需向用户单独索取原文。
export const AI_EXCLUDE = new Set([
    // fx 特效实现（统一入口 fx/69fx-manager.js 保留）
...
// 被省略：fx 特效实现 7 个、modules/16error-capture.js、
// modules/17audio-manager.js、ui/36audio-control.js、index.html。
// 特效统一入口 fx/69fx-manager.js 已包含，接口签名以该文件为准。
