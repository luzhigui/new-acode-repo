// tools/52-version-calibrator.cjs - 光明顶5v5 版本号与字节数批量校准器
...
const SKIP_FILES = new Set([
  'tools/52-version-calibrator.cjs' // 自身最后处理，避免自引用问题
]);
...
function main() {
  ...
  // 处理自身
  try {
    const selfReport = calibrateFile('tools/52-version-calibrator.cjs', targetVersion, dryRun);
