...
### 战斗机制
- **高优先级行动机制**：行动轮询前新增 `beforeActionSelect` 信号，精英角色可声明优先行动（如宋青书苦练）。优先行动不占用本队正常回合，行动后行动方不变，由同队下一个单位继续行动
- 涉及文件：`core/48battle-round.js`、`modules/98elite-sixsects.js`

### Bug 修复
- **苦练优先行动后错误切换行动方**：删除 `kuLianUnit` 引用时漏掉阵营切换判断，导致苦练后敌方回合被跳过。根因是优先级行动标记 `isPriorityAction` 未落地。修复后高优先级单位行动结束不切换 `currentSide`
- 涉及文件：`core/48battle-round.js`
...
- 暴露 `window.selectStage`/`window.forceStopGame`/`window.doManualReset` 供 test runner 调用
- 单元测试扩展至 25 条
>>>>>>> 5de0ae71c41973e0bc4bb9ec356a2dddc3fa4fe3:记录-更改履历.md
