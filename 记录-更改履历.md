...
| 九阳回血 | `99elite-mingjiao.js` | `applyStatChange` → 提交 `heal` 声明 |
| 融会贯通 | `99elite-mingjiao.js` | `applyStatChange` → 提交 `bonusDmg` 声明 |
| 白骨爪联动回复 | `98elite-sixsects.js` | 函数内建临时声明数组，循环结束统一执行 |
| 闪避反击伤害+眩晕 | `49battle-attack-steps.js` | 直接改字段 → 推入 `dodgeDeclarations`，由 `resolveDodgeEffects` 执行 |
...
| 张无忌（ranged/switching/near/ronghui） | `99elite-mingjiao.js` |
| 小昭姐（normal/attaching/attached/returning） | `99elite-mingjiao.js` |
| 小昭妹（normal/transforming/flying/descending/dead） | `99elite-mingjiao.js` |
| 引擎层 FSM 状态判断适配 | `48battle-round.js`、`47battle-attack.js`、`14ui-render.js`、`03battle-utils.js`、`52battle-store.js` |

**四、播放器分层——DOM 操作收口到渲染层**

| 内容 | 文件 |
|------|------|
| 新建 `54renderer.js`（8 个渲染函数） | `54renderer.js` |
| `10player-core.js` 所有 DOM 操作改为调 renderer | `10player-core.js` |
| `53event-handlers.js` 所有 DOM 操作改为调 renderer | `53event-handlers.js` |

**五、回放系统——种子模式 + UI 入口**

| 内容 | 文件 |
|------|------|
| 新增 `startRecordingWithSeed`、`replayFromSeed` | `100-replay.js` |
| 播放器接入录制 | `10player-core.js` |
| 战报弹窗加"下载回放"按钮 | `40main-dialogs.js` |
| 日志工具面板加"导入回放" | `44ui-controls.js` |
| 删除之前错误添加到主按钮区的导入按钮 | `13main-5v5-test.js` |
...
| `ReplayManager` 未加载 | `10player-core.js` 加 `import '../modules/100-replay.js'` |
...
**架构变化**：新增 `beforeStateTransition` 信号（`core/00-event-bus.js`），姐妹组件各自暴露四个 `execute*` 裁判接口，事件监听器只提交声明不再直接执行，由 `core/48battle-round.js` 的裁判统一收集声明并裁定调用。
...
**涉及文件**：`core/49battle-attack-steps.js`、`core/47battle-attack.js`、`core/48battle-round.js`
...
**涉及文件**：`core/03battle-utils.js`、`core/04buff-system.js`、`core/48battle-round.js`、`modules/97elite-imperial.js`、`modules/98elite-sixsects.js`、`modules/99elite-mingjiao.js`
...
- `selectAttackTarget` 中成昆伪装清除逻辑从裁判迁入成昆组件 `modules/97elite-imperial.js`
...
- **文件**：`modules/99elite-mingjiao.js`、`modules/98elite-sixsects.js`
- 已收敛项：
  - 九阳神功回血
  - 白骨爪联动回复
  - 新婚扣血
...
- 目的：引擎层不再硬编码角色名做逻辑判断。
- **文件**：`core/03battle-utils.js`、`core/02unit.js`、`core/48battle-round.js`、`core/01config-5v5-test.js`
- 改动：
  - `03battle-utils.js`：坚盾增量/上限改为读 `_fortifyIncrement`/`_fortifyCap` 字段
  - `02unit.js`：新增两个字段及默认值，`clone()` 同步拷贝
  - `48battle-round.js`：成昆初始化时设置坚盾字段
  - `01config-5v5-test.js`：新增 `FORTIFY_INCREMENT`/`FORTIFY_CAP` 配置项

### 闪避声明化

- 目的：闪避后效果（反击伤害、眩晕、韦一笑吸血）从裁判内联改为声明收集→裁判统一执行。
- **文件**：`core/49battle-attack-steps.js`、`modules/99elite-mingjiao.js`、`core/00-event-bus.js`
- 改动：
  - `49battle-attack-steps.js`：`resolveAttackHit` 改为发射 `onDodge` 信号收集声明，新增 `resolveDodgeEffects` 裁判函数
  - `99elite-mingjiao.js`：韦一笑闪避吸血迁入 `onDodge` 监听器
  - `00-event-bus.js`：`EXECUTION_LAYER` 新增 `ON_DODGE` 层级

### 数值收敛

- 目的：散落各文件的硬编码数值迁入 `CONFIG`。
- **文件**：`core/01config-5v5-test.js`、`core/49battle-attack-steps.js`、`core/03battle-utils.js`
- 新增配置项：
...
- 已替换位置：
  - `49battle-attack-steps.js`：闪避反击比例、圣火令/宝箱掉落率、拒马反伤日志
  - `03battle-utils.js`：战士破防值、远程成长值

### JSDoc 补充

- 目的：核心引擎函数加类型注释。
- **文件**：`core/49battle-attack-steps.js`、`core/47battle-attack.js`、`core/48battle-round.js`
- 已补充函数：
  - `49battle-attack-steps.js`：`resolveAttackHit`、`calcFinalDamage`、`applyAttackResult`、`buildAttackGroup`、`resolveAfterDamageEffects`、`resolveDodgeEffects`
  - `47battle-attack.js`：`processUnitAttack`
  - `48battle-round.js`：`createRoundStepper`
...
- JSDoc：49battle-attack-steps.js 六个核心函数 + 47battle-attack.js processUnitAttack + 48battle-round.js createRoundStepper 加类型注释
```
...
- 修复 `97elite-imperial.js`、`99elite-mingjiao.js` 缺少 `EXECUTION_LAYER` 导入
...
- 全角色技能参数迁入 content/101game-data.json，97/99/23/14ui 接入 getSkillParams
...
### 涉及文件
- `core/48battle-round.js` — 新增 `resolveActionOrder` 行动调度边裁，行动循环重构
- `core/47battle-attack.js` — 免疫拦截改为声明收集，攻击后效果统一结算，死亡边裁调用
- `core/49battle-attack-steps.js` — `selectAttackTarget` 重构、`calcFinalDamage` 重构、新增 `resolveDeaths`/`resolveDamageImmune`/`resolveAfterDamageEffects`、闪避规则注册表
- `core/03battle-utils.js` — 战士破防改为提交 `breakDef` 声明
- `core/04buff-system.js` — 嗜血狂刀/热血奋战/乘风突袭/流星赶月改为声明提交，新增斩杀声明
- `modules/99elite-mingjiao.js` — 韦一笑吸血声明+动态闪避注册、乾坤反弹 `_pendingDeath`、小昭妹飞天免疫声明、小昭永久惑心声明
- `modules/98elite-sixsects.js` — 宋青书叛逆目标+真伤声明、新婚/性奋 `_pendingDeath`、白骨爪 `_pendingDeath`
- `modules/97elite-imperial.js` — 成昆霹雳劲声明+伪装目标声明、鹤笔翁鹿角杖法声明
...
- 涉及文件：`core/03battle-utils.js`（`registerFortifyShield`）、`ui/14ui-render-5v5-test.js`
...
- 涉及文件：`ui/41main-battle.js`、`ui/14ui-render-5v5-test.js`
...
- 涉及文件：`core/01config-5v5-test.js`（`ELITE_SKILLS.phantomThunder.lostHpRatio`）、`ui/14ui-render-5v5-test.js`（技能描述文案）
...
- **清理 48 所有精英相关 import**
- 涉及文件：`core/48battle-round.js`、`core/47battle-attack.js`、`core/03battle-utils.js`、`modules/97elite-imperial.js`、`modules/98elite-sixsects.js`、`modules/99elite-mingjiao.js`
...
- 涉及文件：`core/00-event-bus.js`（新增）、`core/47battle-attack.js`、`core/49battle-attack-steps.js`、`core/51buff-effects.js`、`core/04buff-system.js`
...
- `47battle-attack.js`、`48battle-round.js`、`49battle-attack-steps.js`、`03battle-utils.js`、`51buff-effects.js`、`50battle-shared.js` 全部改为导入 `emitEvent`，不再直接访问 `window._emitEvent`
- 所有 `window._battleEvents` 直接操作改为 `GlobalStore.flushBattleEvents()`
- 涉及文件：`core/50battle-shared.js`、`core/47battle-attack.js`、`core/48battle-round.js`、`core/49battle-attack-steps.js`、`core/03battle-utils.js`、`core/51buff-effects.js`
...
- **大幅修改 ~10 个**：`47battle-attack.js`（-31%）、`48battle-round.js`、`49battle-attack-steps.js`（-11%）、`03battle-utils.js`（+196%）、`44ui-controls.js`、`13main-5v5-test.js`、`04buff-system.js`、`51buff-effects.js`、`50battle-shared.js`、`02unit.js`
...
- 涉及文件：`core/00-event-bus.js`、`core/47battle-attack.js`、`core/49battle-attack-steps.js`、`core/51buff-effects.js`、`modules/23elite-skills.js`、`modules/97elite-imperial.js`、`modules/98elite-sixsects.js`、`modules/99elite-mingjiao.js`

### core 层反向依赖与状态管理重构
- **反向依赖修复**：`core/49battle-attack-steps.js` 删除 `buildAttackGroup` 末尾动态 `import('../fx/21fx-butterfly-spider.js')`；`player/10player-core.js` 在 `handleAttackGroup` 开头补上蝶蛛特效触发
- **文件重编号**：`core/50buff-effects.js` → `core/51buff-effects.js`；同步更新 `core/04buff-system.js` 和 `core/49battle-attack-steps.js` 的 import 路径
- **Store 拆分**：新增 `modules/52battle-store.js`，提取 `createStore`、`battleReducer`、`GAME_STATE_FIELDS`；`player/10player-core.js` 改为 import
- **状态管理统一**：`ui/39main-state.js` 的 `getState`/`setState`/`getPlayerContext` 全部改为从 `GlobalStore` 读写
- **播放器 handler 提取**：新增 `player/53event-handlers.js`，提取 9 个 handler 函数；`player/10player-core.js` 删除本地定义，改为 import
...
- 涉及文件：`core/49battle-attack-steps.js`、`player/10player-core.js`、`core/04buff-system.js`、`core/51buff-effects.js`、`modules/52battle-store.js`、`ui/39main-state.js`、`player/53event-handlers.js`、`modules/23elite-skills.js`
