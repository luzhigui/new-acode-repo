// V6.1.0 | ~11800 bytes | 2026-09-23 妆造字段剥离：SET_FLASH/CLEAR_* 只换 units 引用触发重绘，妆造走 infra/61-view-sheet；删 buffXxxBonus 顶层字段与 STAT_BONUS_CHANGE
export const VER = 'modules/24battle-store.js V6.1.0';
...
import { applyActionToView, clearAllView } from '../infra/61-view-sheet.js';
...
export function createStore(initialState, reducer) {
    ...
    return {
        ...
        dispatch: (action) => {
            ...
            state = next;
            applyActionToView(action, state.units);
...
export function battleReducer(state, action) {
    ...
    switch (action.type) {
        ...
        case STORE_ACTION_TYPES.SET_FLASH: {
            // 妆造字段走视图表（dispatch hook 里 applyActionToView 写入），此处只换 units 引用触发订阅者重绘
        ...
        case STORE_ACTION_TYPES.APPLY_EVENTS: {
            ...
            for (const ev of events) {
                if (ev.eventType === UNIT_EVENT_TYPES.HP_CHANGE || ev.eventType === UNIT_EVENT_TYPES.ZHANG_SWITCH) {
