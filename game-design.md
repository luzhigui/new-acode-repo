...
- 固定站位 **4 号位**。
- **蝶变乾坤**：每回合随机变换职业，记录精通职业，每次变身 +5 生命上限。职业修正累加。
- **变身精通**：每精通一个职业 +1.5 攻 +2 防 +10 血上限。四职业全精通额外 +1 次。
- **乾坤大挪移（衍生版）**：张无忌不在场时，队友受伤触发减伤、治疗和攻击加成。
- **乾坤大挪移（升级版）**：张无忌在场时，保护 2/4/6/8 号位，减伤 30%，反弹 20%（无忌自伤 10%）。
- **永久海克斯**：团队海克斯消失后，小昭单独续上效果（仅限职业匹配的 Buff）。
- **专属强化**：小昭在场时，部分海克斯有强化效果（详见海克斯强化对比表）。
...
- 记录精通职业（去重），变身时职业修正累加（不移除）。
- 每次变身 +5 生命上限（随职业修正同时生效）。
- 变身精通加成：每精通一个职业 +1.5 攻 +2 防 +10 血上限。四职业全精通额外 +1 次。

### 7.2 永久海克斯

- 选 Buff 时同步存储到小昭的 `_permanentBuffs`。
- 规则：团队海克斯还在时用团队的，团队消失后小昭单独续上。
- 仅职业匹配的 Buff 才激活（流云身法、惑人心智无职业限制）。

### 7.3 乾坤大挪移

**衍生版**（张无忌不在场）：队友受伤时触发减伤/治疗/加攻。  
**升级版**（张无忌在场）：保护 2/4/6/8 号位，减伤 30%，反弹原伤害 20%，无忌自伤原伤害 10%。

### 7.4 小昭对团队海克斯的强化

小昭在场时，部分海克斯有强化效果（详见海克斯强化对比表）。强化效果统一收敛至 `ELITE_SKILLS.xiaoZhao.hexEnhance` 配置。
