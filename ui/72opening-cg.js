// ui/72opening-cg.js - 开场CG（三幕黑屏文字，首次进游戏播放）
// V6.0.0 | 2026-09-09 新增开场CG
...
export const VER = 'ui/72opening-cg.js V6.0.0';
...
/* 文字区 */
#cgText{position:absolute;top:15%;left:50%;transform:translateX(-50%);
...
#cgHint{position:absolute;bottom:5.5%;left:0;right:0;text-align:center;
...
export function showOpeningCg(onDone) {
    ...
    const ctx = { overlay, text, hint, act: 0, linesReady: false, switching: false, readyTimer: null, done: false };
    _active = ctx;
    ...
    function fastForward() {
        ...
        text.querySelectorAll('.cg-line,.cg-chapter,.cg-heroes span,#cgSigil').forEach(el => {
            el.style.setProperty('--d', '0s');
            el.style.setProperty('--emberDelay', '0.6s');
            el.style.animationDuration = '0.35s';
        });
        ctx.linesReady = true;
        setTimeout(() => setHint(lastHint()), 450);
    }
    ...
    function nextAct() {
        ...
        setTimeout(() => renderAct(ctx.act + 1), 700);
    }
