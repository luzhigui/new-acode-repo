...
1. **体检规则**：`tests/health-rules/` 下的 rule70-87：
   ...
   - rule81 张无忌近身切换时机回归（仅前排死亡后切换、每场仅 1 次）、rule82 破防显示位置回归（先于伤害行、归属正确）、rule83 流星赶月溅射加攻回归、rule84 苦练提示重复/数值错误回归、rule85 乘风击退换位异常回归（击退距离/落点/换位标记）、rule86 小昭妹/姐不可选期被打回归（飞天/附身窗口内不可被攻击）、rule87 流云身法闪避面板不生效回归（buffDodgeBonus 落到单位数据）
2. **监控器与工具**：
