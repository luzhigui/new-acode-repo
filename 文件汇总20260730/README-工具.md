...
版本: V1.3.0 | 适用: 开发工具维护专项 agent（TraeCode IDE）

本文档是工具维护专项 agent 的专用协议（项目总纲领 `README.md` 的衍生版）。你的职责是**维护 `tools/` 目录下的开发工具**，不修改任何业务逻辑代码。
...
| 本地服务器 | 106b-server.js | 工具链本地静态服务器（零依赖、自动开工具箱；showDirectoryPicker 需 secure-context） |
...
| 战斗 Worker | 116-role-balance-worker.js | 多 kind 分发（balance/elite/stats/baseline），worker 内跑 N 场回报聚合 |
| Worker 池运行器 | 117-shared-worker-runner.js | 复用 116，按核数-1 并行派发 job |
| 启动脚本 | 启动工具箱.bat | 一键起本地服务器 |
| 体检自动化 | health_auto_check.py | Python + Playwright 体检 |
| （预留） | _ui-demo.html | 工具箱 UI 精致化 Demo，暂不使用、有意不入 106/104 清单 |
